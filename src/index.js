...
class App extends React.Component {
  ...
  async selectNetwork(REACH_CONNECTOR_MODE, providerEnv) { // 'ALGO', 'TestNet'
...
class Deployer extends React.Component {
  ...
  async deploy(getPoolDetails) {
    ...
    const myToken = await reach.launchToken(this.props.acc, "AwofTok", "AFT");
    this.setState({view: 'Deploying', ctc});
    const deployerP = ctc.p.Deployer({
      tokenDetails: (async () => { return myToken.id}),
      ready: (async () => {
        const ctcInfoStr = await ctc.getInfo();
        const tokenMeta = await this.props.acc.tokenMetadata(myToken.id)
        thiz.setState({view: 'Deployed', ctcInfoStr, tokenMeta});
      }),
    });
...
class Contributor extends React.Component {
  ...
  async _refreshInfo(acc, ctc) {
    // const runView = async (vname, ...args) => {
      const res = await ctc.views.getUserBalance(this.acc);
    ...
    // const data = {
    //   ...(await runViews([
    //     ['getUserBalance(this.acc)'],
    //   ])),
      // now,
    // };

    this.setState({...res, view: 'ApplicationInfo'});
  }
